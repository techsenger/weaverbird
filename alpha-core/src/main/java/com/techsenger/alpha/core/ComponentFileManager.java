...
class ComponentFileManager {
    ...
    public Path build(ComponentConfig config, Path directoryPath, String extension) throws ComponentException {
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream();
            ...
            addDirectoryToArchive(pathResolver.resolveConfigDirectoryPath(config),
                    DefaultPathManager.CONFIG_DIRECTORY_NAME, zos);
            addDirectoryToArchive(pathResolver.resolveDataDirectoryPath(config),
                    DefaultPathManager.DATA_DIRECTORY_NAME, zos);
            addDirectoryToArchive(pathResolver.resolveDocumentDirectoryPath(config),
                    DefaultPathManager.DOCUMENT_DIRECTORY_NAME, zos);
            addDirectoryToArchive(pathResolver.resolveLegalDirectoryPath(config),
    ...
    public void addComponent(ComponentConfig config, Path zipPath, ZipFile zipfile)
        ...
        var configPath = pathResolver.resolveConfigDirectoryPath(config);
        var dataPath = pathResolver.resolveDataDirectoryPath(config);
        var docPath = pathResolver.resolveDocumentDirectoryPath(config);
        var legalPath = pathResolver.resolveLegalDirectoryPath(config);
    ...
    public void removeComponent(ComponentConfig config) throws ComponentException {
        try {
            deleteDirectory(this.pathResolver.resolveCacheDirectoryPath(config));
            //deleting cache/component-name directory
            deleteDirectoryIfEmpty(this.pathManager.getCacheDirectoryPath(), config);

            deleteDirectory(this.pathResolver.resolveConfigDirectoryPath(config));
            deleteDirectoryIfEmpty(this.pathManager.getConfigDirectoryPath(), config);

            deleteDirectory(this.pathResolver.resolveDataDirectoryPath(config));
            deleteDirectoryIfEmpty(this.pathManager.getDataDirectoryPath(), config);

            deleteDirectory(this.pathResolver.resolveDocumentDirectoryPath(config));
            deleteDirectoryIfEmpty(this.pathManager.getDocumentDirectoryPath(), config);

            deleteDirectory(this.pathResolver.resolveLegalDirectoryPath(config));
            deleteDirectoryIfEmpty(this.pathManager.getLegalDirectoryPath(), config);

            deleteDirectory(this.pathResolver.resolveTempDirectoryPath(config));
            deleteDirectoryIfEmpty(this.pathManager.getTempDirectoryPath(), config);
        } catch (Exception ex) {
    ...
    private void writeArchiveFileToRepo(String filePathStr, byte[] content) throws IOException {
        ...
        var filePath = this.pathManager.getRootDirectoryPath().resolve(filePathStr);
        Files.createDirectories(filePath.getParent());
        try (OutputStream os = Files.newOutputStream(filePath)) {
            os.write(content);
        }
