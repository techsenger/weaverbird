...
class RegistryXmlReader {
    ...
    private class SaxHandler extends DefaultHandler {
        ...
        public void startElement(String uri, String localName, String qName, Attributes attributes)
                throws SAXException {
            switch (qName) {
                case Xml.INSTALL_RESULT_TAG:
                    var info = new InstallResultEntry(
                            Boolean.valueOf(attributes.getValue(Xml.SUCCESSFUL_ATTRIBUTE)));
                    this.registry.setInstallResult(info);
                    break;
                ...
                case Xml.COMPONENT_TAG:
                    var component = new ComponentEntry(attributes.getValue(Xml.NAME_ATTRIBUTE),
                            new Version(attributes.getValue(Xml.VERSION_ATTRIBUTE)));
