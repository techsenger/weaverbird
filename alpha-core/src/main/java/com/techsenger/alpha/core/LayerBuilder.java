...
import com.techsenger.alpha.api.module.ModuleDirective;
import com.techsenger.alpha.api.module.ModuleType;
import com.techsenger.alpha.api.module.ResolvedModuleDirective;
import com.techsenger.alpha.core.component.DefaultComponent;
import com.techsenger.alpha.core.module.DefaultModuleDescriptor;
import com.techsenger.alpha.core.module.DefaultResolvedModuleDirective;
import com.techsenger.alpha.core.war.WarModuleFinder;
...
class LayerBuilder {
    ...
    LayerBuilder(ComponentManager componentManager) {
    ...
    public void build(final DefaultComponent component) throws IOException, Exception {
        ...
        if (logger.isDebugEnabled()) {
            ...
            List<String> warFiles = warModulePaths
                ...
                .map(p -> p.getFileName().toString())
                .collect(Collectors.toList());
            logger.debug("Component has next modules jars: {}, wars: {}",
                    jarFiles, warFiles);
        }
        //collecting data
        ...
        component.getDescriptor().getConfig().getModules().forEach(descriptor -> {
            if (descriptor.getResolvedPath() == null) {
                ((DefaultModuleDescriptor) descriptor).setResolvedPath(
                        this.componentManager.getPathResolver().resolveModuleFilePath(descriptor));
            }
    ...
    private void doBuild(final DefaultComponent component, final ModuleFinder moduleFinder,
        ...
        if (descriptor.usesParentClassLoader()) {
            if (descriptor.getParents().size() != 1) {
    ...
    private String resolveLayerName(ModuleLayer layer) {
        if (layer == Framework.class.getModule().getLayer()) {
            return Framework.getLayerFullName();
        }
    ...
    private Map<String, ModuleLayer> findLayers(DefaultComponent component, String layerName) {
        ...
        if (layerName == null) { //current layer
            ...
            if (nameAndVersion.getSecond() == null) { //find by name
                if (nameAndVersion.getFirst().equals(Framework.getLayerName())) {
                    layersByName.put(Framework.getLayerFullName(), Framework.class.getModule().getLayer());
                } else {
