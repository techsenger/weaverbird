...
public class DefaultServiceTracker implements ServiceTracker {
     ...
     */
    private static class DefaultTrackingRegistration implements ServiceTracker.TrackingRegistration {
    ...
    @Override
    public synchronized void unregisterTracking(TrackingRegistration registration) {
    ...
    /**
     * It is the only method that is not synchronized.
     */
    @Override
    public Optional<Set<ModuleLayer>> getTrackingResult(Class<?> clazz) {
        var result = this.layersByServiceClassName.get(clazz.getName());
        if (result != null) {
    ...
    void initiailize(DefaultComponentManager componentManager) {
