...
class RegistryXmlWriter {
    ...
    public void write(final Path path) throws XMLStreamException, IOException {
        ...
        if (registry.getInstallResult() != null) {
            var result = registry.getInstallResult();
            xmlStreamWriter.writeStartElement(Xml.INSTALL_RESULT_TAG);
            xmlStreamWriter.writeAttribute(Xml.SUCCESSFUL_ATTRIBUTE, String.valueOf(result.wasSuccessful()));
