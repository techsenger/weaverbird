...
import com.techsenger.alpha.api.model.ModuleDirectiveModel;
import static com.techsenger.alpha.api.module.DirectiveType.ALLOWS_EXPORT;
import static com.techsenger.alpha.api.module.DirectiveType.ALLOWS_OPEN;
import static com.techsenger.alpha.api.module.DirectiveType.ALLOWS_READ;
import static com.techsenger.alpha.api.module.DirectiveType.EXPORTS;
import static com.techsenger.alpha.api.module.DirectiveType.OPENS;
import static com.techsenger.alpha.api.module.DirectiveType.READS;
import com.techsenger.alpha.console.gui.settings.ConsoleSettings;
import com.techsenger.alpha.console.gui.settings.LayoutEngine;
...
//        code.append(eol);
    ...
    private void generateComponents(StringBuilder code) {
        for (var layerConfig : this.layerConfigs) {
            if (layerConfig.includedProperty().get() || includedLayerIds.contains(layerConfig.getLayer().getId())) {
                ...
                code.append("] ");
                if (layerConfig.coloredProperty().get()) {
                ...
                for (var moduleConfig : layerConfig.getModules()) {
                    if (moduleConfig.includedProperty().get()
                        ...
                        moduleCode.append("]");
                        if (moduleConfig.coloredProperty().get()) {
                            moduleCode.append(" ");
    ...
    private void generateModuleArrows(StringBuilder code) {
        ...
        for (var layerConfig : this.layerConfigs) {
            ...
            if (layerConfig.includedProperty().get()) {
                for (var moduleConfig: layerConfig.getModules()) {
                    if (moduleConfig.includedProperty().get()) {
    ...
    private void generateModuleBaseArrows(StringBuilder code, LayerConfig layerConfig, ModuleConfig moduleConfig,
        ...
        //reads
        if (moduleConfig.readsProperty().get() && moduleConfig.getResolvedModule().getReads() != null) {
        ...
        if (moduleConfig.exportsProperty().get()) {
        ...
        //opens
        if (moduleConfig.opensProperty().get()) {
            if (moduleConfig.getModule().isNamed()  && !moduleDescriptor.isAutomatic()) {
        ...
        //requires
        if (moduleConfig.requiresProperty().get() && moduleDescriptor.getRequires() != null) {
    ...
    private void generateModuleExtraDirectiveArrows(StringBuilder code, ModuleConfig moduleConfig,
        ...
        if (customDirectives != null) {
            for (var d : customDirectives) {
                switch (d.getType()) {
                    case READS:
                        if (moduleConfig.readsProperty().get()) {
                    ...
                    case OPENS:
                        ...
                        break;
                    case ALLOWS_READ:
                        if (moduleConfig.allowsProperty().get()) {
                            var targetModuleId = includeModule(d.getTargetModule());
                            generateDependencyArrow(code, moduleConfig.getModule().getId(),
                                    targetModuleId, "allows read");
                        }
                        break;
                    case ALLOWS_EXPORT:
                        if (moduleConfig.allowsProperty().get()) {
