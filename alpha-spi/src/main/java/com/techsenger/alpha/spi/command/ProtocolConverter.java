 ...
 */
public class ProtocolConverter implements IStringConverter<Protocol> {

    @Override
    public Protocol convert(String string) {
        if (string.equalsIgnoreCase("http")) {
            return Protocol.HTTP;
        } else if (string.equalsIgnoreCase("rmi")) {
            return Protocol.RMI;
        } else {
            throw new IllegalArgumentException("Uknown protocol");
        }
    }
