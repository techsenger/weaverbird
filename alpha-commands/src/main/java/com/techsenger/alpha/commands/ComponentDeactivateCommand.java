...
public class ComponentDeactivateCommand extends AbstractComponentIdAliasCommand {
    ...
    public void execute(final CommandContext context, MessagePrinter printer) throws Exception {
        ...
        if (getId() != null) {
            descriptor = Framework.getComponentManager().deactivateComponent(getId());
        } else if (getAlias() != null) {
            descriptor = Framework.getComponentManager().deactivateComponent(getAlias());
        } else {
            throw new Exception("Either id or alias must be set");
        }
        printer.printlnMessage(StringUtils.format("Component {}{}{} deactivated", descriptor.getConfig().getName(),
