...
@LocalCommand
@RemoteCommand
@CommandMeta(name = "component:unresolve",
        description = "Unresolves the component by removing all its modules from repo")
public class ComponentUnresolveCommand extends AbstractComponentReferenceCommand {

    @Override
    public String getTitle() {
        return StringUtils.format("Unresolving {}{}{}", getName(), Constants.NAME_VERSION_SEPARATOR, getVersion());
    }
    ...
    public void execute(CommandContext context, MessagePrinter printer) throws Exception {
        ...
        if (!registry.getResolvedComponents().contains(new ComponentEntry(getName(), getVersion()))) {
            throw new CommandSkippedException(StringUtils.format("Component {}{}{} is not resolved",
                    getName(), Constants.NAME_VERSION_SEPARATOR, getVersion()));
        }
        var config = Framework.getComponentManager().unresolveComponent(getName(), getVersion(), printer);
        printer.printlnMessage(StringUtils.format("Component {}{}{} unresolved", getName(),
