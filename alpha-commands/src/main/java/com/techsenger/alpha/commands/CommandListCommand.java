...
public class CommandListCommand extends AbstractCommand {
    ...
    public void execute(final CommandContext context, MessagePrinter printer) throws Exception {
        List<InternalCommandInfo> commandInfos = context.getCommandExecutor().getCommandsByName().values()
                .stream()
                .filter(c -> {
                   if (c.isLocal() && !this.isRemote())  {
                       return true;
                   } else {
                       return (c.isRemote() && this.isRemote());
                   }
                })
                .map(c -> {
                    ...
                    info.setModule(c.getCommandClass().getModule().getName());
                    return info;
                })
