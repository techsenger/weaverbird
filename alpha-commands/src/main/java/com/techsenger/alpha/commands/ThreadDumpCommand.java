...
public class ThreadDumpCommand extends AbstractCommand {
    ...
    public void execute(final CommandContext context, MessagePrinter printer) throws Exception {
        ...
        List<ThreadInfoModel> threadInfo = Framework.getJvmInspector().getThreadInfos();
        //sort them by thread id
        Collections.sort(threadInfo, new Comparator<ThreadInfoModel>() {
            @Override
        ...
        for (ThreadInfoModel info:threadInfo) {
            table.addRow(
                    ++threadCount,
