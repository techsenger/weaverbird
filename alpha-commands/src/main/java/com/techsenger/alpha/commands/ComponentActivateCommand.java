...
public class ComponentActivateCommand extends AbstractComponentIdAliasCommand {
    ...
    public void execute(final CommandContext context, MessagePrinter printer) throws Exception {
        ...
        if (getId() != null) {
            descriptor = Framework.getComponentManager().activateComponent(getId());
        } else if (getAlias() != null) {
            descriptor = Framework.getComponentManager().activateComponent(getAlias());
        } else {
            throw new Exception("Either id or alias must be set");
        }
        printer.printlnMessage(StringUtils.format("Component {}{}{} activated", descriptor.getConfig().getName(),
