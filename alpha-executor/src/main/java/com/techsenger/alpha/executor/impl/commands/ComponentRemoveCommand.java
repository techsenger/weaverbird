...
@LocalCommand
@RemoteCommand
@CommandMeta(name = "component:remove", description = "Removes the component by deleting all its files")
public class ComponentRemoveCommand extends AbstractComponentNameVerCommand {

    @Override
    public String getTitle() {
        return StringUtils.format("Removing {}{}{}", getName(), Constants.NAME_VERSION_SEPARATOR, getVersion());
    }

    @Override
    public void execute(CommandContext context, MessagePrinter printer) throws Exception {
        if (context.isExecutionLocal()) {
            context.getFramework().getComponentManager().removeComponent(getName(), getVersion());
        } else {
            var client = new DomainClient(context.getClient(), context.getSession());
            client.removeComponent(getName(), getVersion());
        }
        printer.printlnMessage(StringUtils.format("Component {}{}{} removed",
