...
@LocalCommand
@RemoteCommand
@CommandMeta(name = "component:start", description = "Starts (deploys and activates) the component.")
public class ComponentStartCommand extends ComponentDeployCommand {

    @Override
    public String getTitle() {
        return StringUtils.format("Starting {}{}{}", getName(), Constants.NAME_VERSION_SEPARATOR, getVersion());
    }

    @Override
    public void execute(final CommandContext context, MessagePrinter printer) throws Exception {
        if (context.isExecutionLocal()) {
            context.getFramework().getComponentManager().startComponent(getName(), getVersion(), getAlias(),
                getParentIds(), getParentAliases(), usesParentClassLoader());
        } else {
            var client = new DomainClient(context.getClient(), context.getSession());
            client.startComponent(getName(), getVersion(), getAlias(),
                getParentIds(), getParentAliases(), usesParentClassLoader());
        }

        printer.printlnMessage(StringUtils.format("Component {}{}{} started", getName(),
