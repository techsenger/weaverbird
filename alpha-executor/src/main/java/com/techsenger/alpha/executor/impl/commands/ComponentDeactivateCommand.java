...
@LocalCommand
@RemoteCommand
@CommandMeta(name = "component:deactivate",
        description = "Deactivates the component by calling all its module activators")
public class ComponentDeactivateCommand extends AbstractComponentIdAliasCommand {
    ...
    public String getTitle() {
        ...
        return StringUtils.format("Deactivating {}", identifier);
    }

    @Override
    public void execute(final CommandContext context, MessagePrinter printer) throws Exception {
        ComponentConfigDto config = null;
        if (getId() != null) {
            if (context.isExecutionLocal()) {
                config = context.getFramework().getComponentManager().deactivateComponent(getId()).getConfig();
            } else {
                var client = new DomainClient(context.getClient(), context.getSession());
                config = client.deactivateComponent(getId()).getConfig();
            }
        } else if (getAlias() != null) {
            if (context.isExecutionLocal()) {
                config = context.getFramework().getComponentManager().deactivateComponent(getAlias()).getConfig();
            } else {
                var client = new DomainClient(context.getClient(), context.getSession());
                config = client.deactivateComponent(getAlias()).getConfig();
            }
        } else {
            throw new Exception("Either id or alias must be set");
        }
        printer.printlnMessage(StringUtils.format("Component {}{}{} deactivated", config.getName(),
