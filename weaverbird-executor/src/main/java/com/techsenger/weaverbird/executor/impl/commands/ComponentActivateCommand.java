...
@LocalCommand
@RemoteCommand
@CommandMeta(name = "component:activate", description = "Activates the component by calling all its module activators")
public class ComponentActivateCommand extends AbstractComponentIdAliasCommand {
    ...
    public String getTitle() {
        ...
        return StringUtils.format("Activating {}", identifier);
    }
    ...
    public void execute(final CommandContext context, MessagePrinter printer) throws Exception {
        ...
        if (getId() != null) {
            if (context.isExecutionLocal()) {
                config = context.getFramework().getComponentManager().activateComponent(getId()).getConfig();
            } else {
                var client = new DomainClient(context.getClient(), context.getSession());
                config = client.activateComponent(getId()).getConfig();
            }
        } else if (getAlias() != null) {
            if (context.isExecutionLocal()) {
                config = context.getFramework().getComponentManager().activateComponent(getAlias()).getConfig();
            } else {
                var client = new DomainClient(context.getClient(), context.getSession());
                config = client.activateComponent(getAlias()).getConfig();
            }
        } else {
            throw new Exception("Either id or alias must be set");
        }
        printer.printlnMessage(StringUtils.format("Component {}{}{} activated", config.getName(),
