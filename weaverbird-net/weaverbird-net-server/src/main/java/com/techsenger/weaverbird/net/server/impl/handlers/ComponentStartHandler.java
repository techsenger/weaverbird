 ...
 */
@Endpoint(Endpoints.COMPONENT_START)
public class ComponentStartHandler extends AbstractEndpointHandler<FrameworkRequestContext, ComponentStartRequest> {

    public ComponentStartHandler() {
        super(ComponentStartRequest.class);
    }

    @Override
    public Response handle(FrameworkRequestContext context, ComponentStartRequest request) throws Exception {
        var descriptor =  context.getFramework().getComponentManager()
                .startComponent(request.getName(), request.getVersion(), request.getAlias(),
