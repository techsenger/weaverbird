 ...
 */
@Endpoint(Endpoints.SESSION_LIST)
public class SessionListHandler extends AbstractEndpointHandler<FrameworkRequestContext, Request> {
    ...
    public Response handle(FrameworkRequestContext context, Request request) throws Exception {
        var sessions = context.getServer().getSessionManager().getSessions().stream().map(s -> ServerSessionDto.of(s))
                .toList();
