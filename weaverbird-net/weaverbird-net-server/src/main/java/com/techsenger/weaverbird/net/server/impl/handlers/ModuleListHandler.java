 ...
 */
@Endpoint(Endpoints.MODULE_LIST)
public class ModuleListHandler extends AbstractEndpointHandler<FrameworkRequestContext, Request> {

    public ModuleListHandler() {
        super(Request.class);
    }

    @Override
    public Response handle(FrameworkRequestContext context, Request request) throws Exception {
        var modulesByLayerName = context.getFramework().getJvmInspector().getModulesInfo().getModulesByLayerName();
        return new ModuleListResponse(modulesByLayerName);
    }
}
