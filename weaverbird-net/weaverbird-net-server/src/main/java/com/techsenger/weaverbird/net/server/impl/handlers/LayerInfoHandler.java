 ...
 */
@Endpoint(Endpoints.LAYER_INFO)
public class LayerInfoHandler extends AbstractEndpointHandler<FrameworkRequestContext, Request> {

    public LayerInfoHandler() {
        super(Request.class);
    }

    @Override
    public Response handle(FrameworkRequestContext context, Request request) throws Exception {
        var response = new LayerInfoResponse(context.getFramework().getJvmInspector().getLayersInfo());
        return response;
    }
}
