 ...
 */
@Endpoint(Endpoints.THREAD_INFO_LIST)
public class ThreadInfoListHandler extends AbstractEndpointHandler<FrameworkRequestContext, Request> {

    public ThreadInfoListHandler() {
        super(Request.class);
    }

    @Override
    public Response handle(FrameworkRequestContext context, Request request) throws Exception {
        var infos = context.getFramework().getJvmInspector().getThreadInfos();
        return new ThreadInfoListResponse(infos);
    }
