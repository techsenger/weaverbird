 ...
 */
@Endpoint(Endpoints.THREAD_LIST)
public class ThreadListHandler extends AbstractEndpointHandler<FrameworkRequestContext, Request> {

    public ThreadListHandler() {
        super(Request.class);
    }

    @Override
    public Response handle(FrameworkRequestContext context, Request request) throws Exception {
        var threads = context.getFramework().getJvmInspector().getThreads();
